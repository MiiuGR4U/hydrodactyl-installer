...
This guide provides step-by-step instructions for manually installing the Wings Daemon on your game server nodes. Wings is the game server management daemon that communicates with the Hydrodactyl Panel and manages Docker containers for game servers.
...
6. [Step 4: Download and Install Wings](#step-4-download-and-install-Wings)
7. [Step 5: Configure Wings](#step-5-configure-Wings)
...
Wings uses a specific Docker network. You can customize this in the config later, but the defaults are usually fine.

```bash
# Verify Docker network (will be created automatically by Wings)
docker network ls
```
...
Wings runs as a dedicated user (UID 8888):
...
Wings requires SSL when connecting to a panel that uses HTTPS. You have two options:
...
# Create symlinks for Wings
mkdir -p /etc/Wings
ln -sf /etc/letsencrypt/live/your-daemon-domain.com/fullchain.pem /etc/Wings/certificate.pem
ln -sf /etc/letsencrypt/live/your-daemon-domain.com/privkey.pem /etc/Wings/certificate.key

# Set ownership
chown 8888:8888 /etc/Wings/certificate.pem /etc/Wings/certificate.key
...
openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
    -keyout /etc/Wings/certificate.key \
    -out /etc/Wings/certificate.pem \
...
#!/bin/bash
echo "[$(date)] Certificate renewed, restarting Wings..." >> /var/log/Wings-certbot-renewal.log
...
**Note:** Wings must run as root to manage Docker containers. The game servers themselves run as UID 8888.
...
# Wings API (if using custom port)
ufw allow 8080/tcp
...
Edit `/etc/Wings/config.yml` and update the authentication values:
...
1. **Invalid configuration:**
   ...
   cat /etc/Wings/config.yml | head -20
   ```
...
**Verify token:**
1. In Panel, go to Node â†’ Settings
2. Click **Reset Daemon Token** if needed
3. Update `/etc/Wings/config.yml`
4. Restart Wings
...
Wings uses Docker images from the [Parkervcp Eggs repository](https://github.com/parkervcp/eggs). Import common eggs in your Panel:

1. Go to **Admin** â†’ **Nests**
...
Now that Wings is installed and connected:
...
**Congratulations!** Your Wings Daemon is now installed and ready to manage game servers.
