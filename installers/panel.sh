...
main() {
  ...
  echo ""
  output "Ã°Å¸Å½â€° Your Hydrodactyl Panel has been installed successfully!"
  echo ""
  ...
  output "Admin Password: ${COLOR_BLUE_THEME}**hidden** (hope you remember it!)${COLOR_NC}"
  echo ""
  output "Database credentials saved to: ${COLOR_BLUE_THEME}/root/.config/Hydrodactyl/db-credentials${COLOR_NC}"
  echo ""
  ...
  if [ -n "$PANEL_API_KEY" ]; then
  output "──────────────────────────────────────────────────────────────────────"
    output "  API Key for Wings Setup"
  output "──────────────────────────────────────────────────────────────────────"
    output "API Key: ${COLOR_BLUE_THEME}${PANEL_API_KEY}${COLOR_NC}"
    echo ""
    output "Save this API key! You can use it to automatically configure"
    output "Wings without manually entering node ID and token."
    output "When running Wings.sh, enter this API key when prompted."
    echo ""
  fi

  if [ "$INSTALL_AUTO_UPDATER" == true ]; then
    output "Ã¢Å“â€¦ Auto-updater is enabled and will check for updates hourly."
    echo ""
  fi

  output "Service Commands:"
  output "  ${COLOR_BLUE_THEME}systemctl status pteroq${COLOR_NC}    - Panel queue worker"
  output "  ${COLOR_BLUE_THEME}systemctl status nginx${COLOR_NC}    - Web server"
  output "  ${COLOR_BLUE_THEME}systemctl status mariadb${COLOR_NC}  - Database"
  echo ""
